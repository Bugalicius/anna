...
class AgenteRetencao:
    ...
    def _detectar_tipo_remarcacao(self) -> str:
        ...
        self.consulta_atual = agenda  # guarda dados para exibir ao paciente
    ...
    def _fluxo_remarcacao(self, msg: str) -> list[str]:
        ...
        if self.etapa == "inicio":
            ...
            # Se encontramos o agendamento no Dietbox, exibimos a data ao paciente
            # para que ele não precise lembrar — e seguimos direto para preferência
            if self.consulta_atual:
                try:
                    from datetime import datetime as _dt
                    dt_atual = _dt.fromisoformat(self.consulta_atual["inicio"])
                    data_fmt = dt_atual.strftime("%d/%m/%Y")
                    hora_fmt = dt_atual.strftime("%Hh")
                    return [
                        MSG_CONSULTA_ATUAL_ENCONTRADA.format(
                            data=data_fmt,
                            hora=hora_fmt,
                            modalidade=self.modalidade,
                        )
                    ...
                    logger.error("Erro ao formatar consulta_atual para MSG_CONSULTA_ATUAL_ENCONTRADA: %s", e)
