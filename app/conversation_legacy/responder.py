...
async def gerar_resposta(state: dict, plano: dict, resultado_tool: dict | None) -> list:
    ...
    if action == "execute_tool" and plano.get("tool") in ("consultar_slots", "consultar_slots_remarcar"):
        if resultado_tool and resultado_tool.get("slots"):
            ...
            if plano.get("tool") == "consultar_slots_remarcar":
                if resultado_tool.get("slots_mesma_semana"):
                    return [_build_slot_buttons(slots, "Tive desistências essa semana:")]
                if aviso:
                    return [_build_slot_buttons(slots, aviso)]
                return [_build_slot_buttons(slots)]
    ...
    # ── B2B ───────────────────────────────────────────────────────────────────
    if action == "respond_b2b":
        return [
            "Olá! Este canal é dedicado ao atendimento individual da nutricionista Thaynara Teixeira 💚\n\n"
            "Para parcerias ou contratos corporativos, entre em contato diretamente pelo WhatsApp da equipe."
        ]

    # ── Agendamento para terceiro ──────────────────────────────────────────────
    if action == "handle_terceiro":
        draft = plano.get("draft_message")
        if draft:
            return [draft]
        return ["Para quem é a consulta? Me conta o nome completo da pessoa que vai se consultar 😊"]
...
def _answer_from_kb(topico: str, cd: dict) -> str:
    ...
    if topico == "indicacao":
        return (
            "Temos um programa de indicação sim! 💚\n\n"
            "Quando você indica alguém que efetiva uma consulta com a Thaynara, você recebe benefícios especiais. "
            "Os detalhes variam conforme o período — mas é uma ótima forma de ajudar alguém e ainda ser reconhecida por isso!\n\n"
            "Quando indicar alguém, me diz o nome do indicado para registrarmos direitinho 😊"
        )
    if topico == "dupla_familia":
        return (
            "Sim! Quando duas pessoas fecham juntas (casal, mãe e filha, irmãs, amigas...), "
            "cada uma ganha *10% de desconto* no valor total 💚\n\n"
            "As consultas são agendadas em horários sequenciais no mesmo dia.\n\n"
            "Quer verificar a disponibilidade para as duas?"
        )
...
def _word_overlap_score(query: str, candidate: str) -> float:
    """Jaccard-like word overlap between two normalized strings."""
    q_words = set(query.split())
    c_words = set(candidate.split())
    if not q_words or not c_words:
        return 0.0
    # Remove very short/common words
    _STOP = {"e", "o", "a", "de", "da", "do", "em", "um", "uma", "que", "se", "no", "na"}
    q_words -= _STOP
    c_words -= _STOP
    if not q_words or not c_words:
        return 0.0
...
def _answer_faq_from_message(message: str) -> str | None:
    ...
    for item in kb.faq_combinado():
        ...
        if not faq_q or not resposta:
            continue
        score = _word_overlap_score(pergunta, faq_q)
        if score > best_score:
            best_score = score
            best_answer = resposta
    if best_score >= 0.6:
        return best_answer
    return None
