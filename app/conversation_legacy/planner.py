...
RESPOND_B2B               = "respond_b2b"
HANDLE_TERCEIRO           = "handle_terceiro"
...
def _is_b2b(texto: str | None) -> bool:
    """Detecta contato corporativo/B2B que não deve entrar no fluxo normal."""
    t = _normalizar_texto_simples(texto)
    # Marcadores fortes
    if any(w in t for w in ("cnpj", "b2b", "plano corporativo", "contrato corporativo", "parceria comercial")):
        return True
    # Combinação empresa + intenção comercial
    if any(w in t for w in ("empresa", "companhia")) and any(
        w in t for w in ("contratar", "parceria", "colaboradores", "funcionarios", "oferta comercial")
    ):
        return True
    return False


def _is_terceiro_explicito(texto: str | None) -> bool:
    """Detecta quando o contato quer marcar consulta para outra pessoa."""
    t = _normalizar_texto_simples(texto)
    relacoes = (
        "minha mae", "minha filha", "meu filho", "meu marido", "minha esposa",
        "minha irma", "meu irmao", "minha amiga", "meu amigo",
        "minha neta", "meu neto", "minha sobrinha", "meu sobrinho",
        "minha avó", "minha avo", "meu pai",
    )
    marcacao = ("consulta", "marcar", "agendar", "horario")
    if any(r in t for r in relacoes) and any(a in t for a in marcacao):
        return True
    # "quero marcar para ela/ele"
    import re as _re
    if _re.search(r"(marcar|agendar|consulta)\s+(para|pra)\s+(ela|ele)\b", t):
        return True
    return False


def _quer_contato_thaynara(texto: str | None) -> bool:
    """Detecta quando quem_marca quer repassar a consulta para o paciente real ligar diretamente."""
    t = _normalizar_texto_simples(texto)
    return any(p in t for p in (
        "fala com ela", "passa o contato", "manda o numero", "manda o contato",
        "ela liga", "ela pode ligar", "ela mesma liga",
    ))
...
def _override_deterministic(turno: dict, state: dict) -> dict | None:
    ...
    # ── B2B: resposta única + notificação Breno ───────────────────────────────
    if _is_b2b(raw_msg):
        return _plano(
            RESPOND_B2B,
            new_status="concluido",
            update_flags={"b2b": True, "ignorar_remarketing": True},
        )

    # ── Agendamento para terceiro ─────────────────────────────────────────────
    if _is_terceiro_explicito(raw_msg) and not flags.get("modo_terceiro") and not cd.get("nome_terceiro"):
        return _plano(
            HANDLE_TERCEIRO,
            update_flags={"modo_terceiro": True},
            draft_message="Claro! Para quem é a consulta? Me conta o nome completo da pessoa que vai se consultar 😊",
        )
    if _quer_contato_thaynara(raw_msg) and flags.get("modo_terceiro"):
        from app.knowledge_base import kb as _kb
        return _plano(
            ANSWER_QUESTION,
            ask_context="contato_thaynara",
            draft_message=(
                f"Claro! O número da Thaynara é {_kb.contatos['numero_nutri_publico']}. "
                "Ela pode entrar em contato diretamente para marcar 💚"
            ),
        )
    if flags.get("modo_terceiro") and not cd.get("nome_terceiro"):
        nome_terceiro = turno.get("nome")
        if nome_terceiro:
            primeiro = nome_terceiro.split()[0]
            return _plano(
                ASK_FIELD,
                ask_context="objetivo",
                update_data={"nome_terceiro": nome_terceiro, "nome": nome_terceiro},
                update_flags={"modo_terceiro": False},
                draft_message=(
                    f"Anotei! Vou cuidar do agendamento da {primeiro} com carinho 💚\n\n"
                    "Qual é o principal objetivo dela com o acompanhamento nutricional?"
                ),
            )
        return _plano(
            HANDLE_TERCEIRO,
            draft_message="Pode me dizer o *nome completo* da pessoa que vai se consultar? 😊",
        )
    ...
    if (
        ...
        and turno.get("topico_pergunta") in (
            "pagamento", "planos", "modalidade", "politica", "indicacao", "dupla_familia"
        )
