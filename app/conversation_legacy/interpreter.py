...
- Se o paciente mencionar que é gestante/grávida ou menor de 16 anos, use intent="tirar_duvida", tem_pergunta=true, topico_pergunta="politica". Não trate como agendamento normal.
...
_TOPICOS = {"pagamento", "planos", "modalidade", "politica", "clinica", "indicacao", "dupla_familia"}
...
def _heuristic_turno(text: str, state: dict) -> dict:
    ...
    if any(w in t for w in ("gestante", "grávida", "gravida", "gravidez", "menor de 16", "menor que 16")):
        turno["intent"] = "tirar_duvida"
        turno["tem_pergunta"] = True
        turno["topico_pergunta"] = "politica"
        return turno
    ...
    # Detecção de programa de indicação
    _indicacao_words = (
        "indicação", "indicacao", "programa de indicacao", "programa de indicação",
        "indico alguem", "indicar alguem", "recompensa indicacao", "bonus indicacao",
        "quem indico", "quem eu indico",
    )
    if any(w in t for w in _indicacao_words):
        turno["tem_pergunta"] = True
        turno["topico_pergunta"] = "indicacao"

    # Detecção de consulta em dupla/família
    _dupla_words = (
        "em dupla", "duas pessoas", "em dois", "nós dois", "nos dois", "nós duas",
        "nos duas", "casal", "dois juntos", "duas juntas", "pra dois", "para dois",
        "pra nós dois", "para nos dois",
    )
    _acao_marcacao_dupla = ("consulta", "marcar", "agendar", "desconto")
    if any(w in t for w in _dupla_words) and any(a in t for a in _acao_marcacao_dupla):
        turno["tem_pergunta"] = True
        turno["topico_pergunta"] = "dupla_familia"
